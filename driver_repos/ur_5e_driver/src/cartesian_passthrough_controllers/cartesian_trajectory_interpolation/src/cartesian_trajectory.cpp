...
CartesianTrajectory::CartesianTrajectory(const cartesian_passthrough_msgs::msg::CartesianTrajectory& ros_trajectory)
{
...
bool CartesianTrajectory::init(const cartesian_passthrough_msgs::msg::CartesianTrajectory& ros_trajectory)
{
  ...
  if (rclcpp::Duration(i->time_from_start).seconds() >=
      rclcpp::Duration(std::next(i)->time_from_start).seconds())
    {
      return false;
    }
    ...
    CartesianTrajectorySegment s(
        rclcpp::Duration(i->time_from_start).seconds(),
        state,
        rclcpp::Duration(std::next(i)->time_from_start).seconds(),
        next_state);
  }
